...
const MAX_RESULTS = 5
const NONE_LABEL = 'No cover featured'

function displayText(selected) {
  return selected?.title ?? NONE_LABEL
}
...
export default function BookPicker({ books, value, onChange, excludeIds = [], placeholder = 'Search books…' }) {
  ...
  const [query, setQuery] = useState(displayText(selected))
  ...
  const searching = q !== '' && q !== NONE_LABEL.toLowerCase()
  const results = books
    .filter(b => b.id === value || !excludeIds.includes(b.id))
    .filter(b => !searching || b.title.toLowerCase().includes(q))
    .slice(0, MAX_RESULTS)
    ...
    <div ref={wrapperRef} className="relative">
      <div className="relative">
        <input
          ...
          onKeyDown={e => { if (e.key === 'Escape') { setOpen(false); setQuery(displayText(selected)) } }}
        ...
        <div className="absolute z-20 mt-1 w-full bg-white border rounded shadow-lg overflow-hidden">
          <button
            ...
            className={`block w-full text-left px-3 py-2 text-sm hover:bg-gray-50 transition-colors ${
              !value ? 'text-deep-space-blue font-medium' : 'text-gray-500'
            }`}
          >
            {NONE_LABEL}
          </button>
